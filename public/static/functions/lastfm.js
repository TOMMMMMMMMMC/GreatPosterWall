;(function () {
  ...
  $(document).ready(function () {
    ...
    $('#lastfm_reload').on('click', function () {
      ...
      $.get(
        'user.php?action=lastfm_clear_cache&username=' +
          username +
          '&uid=' +
          $.urlParam('id'),
        function (response) {}
  ...
  function getTasteometer(div) {
    if ($('#lastfm_stats').attr('data-uid')) {
      ...
      $.get(
        'user.php?action=lastfm_compare&username=' + username,
        function (response) {
            ...
            if (json != null && json['error']) {
              ...
              if (compatibility < 0 || compatibility > 100) {
                ...
                if (compatibility < 50) {
                  background =
                    'rgb(255, ' +
                    Math.floor((255 * compatibility) / 50) +
                    ', 0)'
                } else {
                  background =
                    'rgb(' +
                    Math.floor((1 - (compatibility - 50) / 50) * 255) +
                    ', 255, 0)'
                }
                tasteometerHtml +=
                  compatibility +
                  '%\r\
						<li class="lastfm_essential">\r\
							<div id="lastfm_compatibilitybar_container">\n\
								<div id="lastfm_compatibilitybar" style="width: ' +
                  compatibility +
                  '%; background: ' +
                  background +
                  ';">\n\
              ...
              if (j['artists']['matches'] != 0) {
                sharedArtistsHtml +=
                  '<li>Shared artists:</li><li><ul class="nobullet">'
                var k = initialCount
                if (a.length < 3) {
                  k = a.length
                }
                for (var i = 0; i < k; i++) {
                  sharedArtistsHtml +=
                    '<li><a href="artist.php?artistname=' +
                    escapeAmpUrl(a[i]['name']) +
                    '">' +
                    escapeHtml(a[i]['name']) +
                    '</a></li>'
                }
                if (a.length > 3) {
                  for (i = 3; i < a.length; i++) {
                    sharedArtistsHtml +=
                      '<li class="hidden"><a href="artist.php?artistname=' +
                      escapeAmpUrl(a[i]['name']) +
                      '">' +
                      escapeHtml(a[i]['name']) +
                      '</a></li>'
                  ...
                  sharedArtistsHtml +=
                    '<li><a href="#sharedartists" id="lastfm_expand" onclick="return false" class="brackets">Expand</a></li>'
                }
                ...
                sharedArtists = ' '
                sharedArtistsHtml +=
                  '<li class="lastfm_expand"><a href="#sharedartists" id="lastfm_expand" onclick="return false" class="brackets">Expand</a></li>'
              }
              tasteometerHtml += '</li>'
  ...
  function getLastPlayedTrack(div) {
    ...
    $.get(
      'user.php?action=lastfm_last_played_track&username=' + username,
      function (response) {
        var html = ''
        if (response && response != 'false') {
          ...
          if (json != null && json['error']) {
            ...
            html +=
              '<a href="artist.php?artistname=' +
              escapeAmpUrl(json['artist']['#text']) +
              '">' +
              escapeHtml(json['artist']['#text']) +
              '</a> - <a href="torrents.php?artistname=' +
              escapeAmpUrl(json['artist']['#text']) +
              '&filelist=' +
              escapeAmpUrl(json['name']) +
              '">' +
              escapeHtml(json['name']) +
              '</a>'
            html += '</li>'
  ...
  function getTopArtists(div) {
    ...
    $.get(
      'user.php?action=lastfm_top_artists&username=' + username,
      function (response) {
        var html
        if (response && response != 'false') {
          var json = JSON.parse(response)
          if (json != null && json['error']) {
            ...
            if (j.length > 3) {
              ...
              html +=
                '<li><a href="#topartists" id="lastfm_expand" onclick="return false" class="brackets">Expand</a></li>'
            }
            html += '</ul>'
            html += '</li>'
            topArtists = html
  ...
  function getTopAlbums(div) {
    ...
    $.get(
      'user.php?action=lastfm_top_albums&username=' + username,
      function (response) {
        var html
        if (response && response != 'false') {
          var json = JSON.parse(response)
          if (json != null && json['error']) {
            ...
            for (var i = 0; i < k; i++) {
              html +=
                '<li><a href="artist.php?artistname=' +
                escapeAmpUrl(j[i]['artist']['name']) +
                '">' +
                escapeHtml(j[i]['artist']['name']) +
                '</a> - <a href="torrents.php?searchstr=' +
                escapeAmpUrl(j[i]['name']) +
                '">' +
                escapeHtml(j[i]['name']) +
                '</a></li>'
            ...
            if (j.length > 3) {
              ...
              html +=
                '<li><a href="#topalbums" id="lastfm_expand" onclick="return false" class="brackets">Expand</a></li>'
            }
            html += '</ul>'
            html += '</li>'
            topAlbums = html
  ...
  function getTopTracks(div) {
    ...
    $.get(
      'user.php?action=lastfm_top_tracks&username=' + username,
      function (response) {
        var html
        if (response && response != 'false') {
          var json = JSON.parse(response)
          if (json != null && json['error']) {
            ...
            if (j.length > 3) {
              ...
              html +=
                '<li><a href="#toptracks" id="lastfm_expand" onclick="return false" class="brackets">Expand</a></li>'
            }
            html += '</ul>'
            html += '</li>'
            topTracks = html
