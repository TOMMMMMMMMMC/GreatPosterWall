...
?>
<? View::show_header(Lang::get('login', 'login')); ?>
<span id="no-cookies" class="hidden warning">你似乎禁用了 Cookies。</span>
<noscript><span class="warning"><?= SITE_NAME ?>本页面需要启用 JavaScript 才能正常工作，请为你的浏览器启用 JavaScript。</span><br /></noscript>
<?
...
?>
    <form class="auth_form" name="login" id="loginform" method="post" action="login.php">
        <div id="form-body">
            ...
            if (isset($Err)) {
            ?>
                <br /><span class="warning"><?= $Err ?></span>
            <?
            }
            ?>
            <? if ($Attempts > 0) { ?>
            ...
            <? if (isset($_GET['invalid2fa'])) { ?>
                <span class="warning"><?= Lang::get('login', 'warning') ?> </span><br />
            <?    } ?>
            ...
            <div id="login-table">
                ...
                <div class="username-input">
                    <input type="text" name="username" id="username" class="inputtext" required="required" maxlength="20" autofocus="autofocus" placeholder="Username" />
                </div>
                <div class="space"></div>
                ...
                <div id="remember-login">
                    ...
                    <label for="keeplogged" id="keeplogged-label"><?= Lang::get('login', 'remember_me') ?></label>
                    <input type="submit" name="login" value="<?= Lang::get('login', 'login') ?>" class="submit" id="login-btn" />
                </div>
                <div class="space"></div>
...
?>
    <span class="warning"><?= Lang::get('login', 'warning_disable_before') ?><?= time_diff($BannedUntil) ?><?= Lang::get('login', 'warning_disable_after') ?></span><br />
<?
}

if ($Attempts > 0) {
?>
    <span id="find-pw"><?= Lang::get('login', 'forget_pw') ?> <a href="login.php?act=recover" class="tooltip" title="Recover your password"><?= Lang::get('login', 'find_pw') ?></a></span>
<?
}
?>
