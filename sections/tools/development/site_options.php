...
if (!check_perms('admin_manage_permissions')) {
    View::show_header(Lang::get('tools', 'h2_site_options'));
    $DB->query("SELECT Name, Value, Comment FROM site_options");
?>
    <div class="header">
        <h2><?= Lang::get('tools', 'h2_site_options') ?></h2>
    </div>
    <div class="table_container border">
        <table width="100%">
            <tr class="colhead">
                <td><?= Lang::get('tools', 'name') ?></td>
                <td><?= Lang::get('tools', 'value') ?></td>
                <td><?= Lang::get('tools', 'comment') ?></td>
            </tr>
            <?
            $Row = 'a';
...
<div class="table_container border">
    <table width="100%">
        <tr class="colhead">
            <td>
                <span class="tooltip" title="<?= Lang::get('tools', 'words_must_be_separated_by_underscores') ?>"><?= Lang::get('tools', 'name') ?></span>
            </td>
            <td><?= Lang::get('tools', 'value') ?></td>
            <td><?= Lang::get('tools', 'comment') ?></td>
