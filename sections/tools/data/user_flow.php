...
View::show_header(Lang::get('tools', 'h2_user_flow'));
$DB->set_query_id($RS);
?>
<div class="thin">
	<? if (!isset($_GET['page'])) { ?>
	...
	<table width="100%" id="user_flow_table">
		<tr class="colhead">
			<td><?= Lang::get('tools', 'date') ?></td>
			<td>(+) <?= Lang::get('tools', 'joined') ?></td>
			<td>(-) <?= Lang::get('tools', 'manual') ?></td>
			<td>(-) <?= Lang::get('tools', 'ratio') ?></td>
			<td>(-) <?= Lang::get('tools', 'inactivity') ?></td>
			<td>(-) <?= Lang::get('tools', 'total') ?></td>
			<td><?= Lang::get('tools', 'net_growth') ?></td>
		</tr>
		<?
