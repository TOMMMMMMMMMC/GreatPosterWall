...
View::show_header(Lang::get('tools', 'application_management'));
        ...
        <li>
            <form>
                ...
                <label for="searchemail"><?= Lang::get('tools', 'application_email') ?>:</lable>
                    <input id="searchemail" spellcheck="false" onfocus="if (this.value == 'Email') { this.value = ''; }" onblur="if (this.value == '') { this.value = 'Email'; }" value=<?= $Email ? $Email : "Email" ?> placeholder="Email" type="text" name="email">
            </form>
        </li>
        <li>
            <form>
                ...
                <label for="searchip"><?= Lang::get('tools', 'application_ip') ?>:</lable>
                    <input id="searchip" spellcheck="false" onfocus="if (this.value == 'IP') { this.value = ''; }" onblur="if (this.value == '') { this.value = 'IP'; }" value=<?= $IP ? $IP : "IP" ?> placeholder="IP" type="text" name="ip">
            </form>
        </li>
    </ul>
</div>
...
<table width="100%" id="application_record_list">
    <tr class="colhead">
        <td id="th_group_number" class="group_number" title="<?= Lang::get('tools', 'group_number') ?>"><?= Lang::get('tools', 'group_number') ?></td>
        <!-- <td style="width: 30px;">介绍</td> -->
        <td id="th_id">ID</td>
        <td id="th_submitted_at"><?= Lang::get('tools', 'submitted_at') ?></td>
        <td id="th_status"><?= Lang::get('tools', 'status') ?></td>
        <td id="th_operator"><?= Lang::get('tools', 'operator') ?></td>
        <td id="th_email"><?= Lang::get('tools', 'email') ?></td>
        <td id="th_ip">IP</td>
        <td id="th_opinion"><?= Lang::get('tools', 'opinion') ?></td>
        <td id="th_tips"><?= Lang::get('tools', 'tips') ?></td>
        <td id="th_access"><?= Lang::get('tools', 'access') ?></td>
    ...
    ?>
        <tr class="row<?= $Row ?>">
            <form class="manage_form" name="apply_list" action="" method="post" onsubmit="return submit_sure()">
                ...
                <td class="group_number" onclick="toggleShow('detail<?= $ID ?>')">
                    <p><?= Lang::get('tools', 'group') ?> <?= ($ID % $groupCount) + 1 ?></p>
                </td>
                <!-- <td>
            <a href="#" title="<?= base64_decode($introduction) ?>">介绍</a>
                </td> -->

                <td onclick="toggleShow('detail<?= $ID ?>')">
                    <p><?= $ID ?></p>
                </td>
                <td onclick="toggleShow('detail<?= $ID ?>')">
                    <p><?= $ts ?></p>
                ...
                <td>
                    <p><a href="user.php?action=search&username=&joined=on&join1=&join2=&enabled=&email=<?= $email ?>&lastactive=on&lastactive1=&lastactive2=&class=&ip=&lockedaccount=any&secclass=&ip_history=on&email_history=on&ratio=equal&ratio1=&ratio2=&donor=&disabled_invites=&uploaded=equal&uploaded1=&uploaded2=&disabled_uploads=&invites=equal&invites1=&invites2=&downloaded=equal&downloaded1=&downloaded2=&warned=&invitees=off&invitees1=&invitees2=&snatched=off&snatched1=&snatched2=&comment=&passkey=&avatar=&lastfm=&tracker_ip=&stylesheet=&cc_op=equal&cc=&matchtype=fuzzy&order=Joined&way=Descending&emails_opt=equal&email_cnt=" title="<?= $email ?>"><?= $email ?></a></p>
                </td>
                <td>
                    <p><a href="user.php?action=search&username=&joined=on&join1=&join2=&enabled=&email=&lastactive=on&lastactive1=&lastactive2=&class=&ip=<?= $ip ?>&lockedaccount=any&secclass=&ip_history=on&email_history=on&ratio=equal&ratio1=&ratio2=&donor=&disabled_invites=&uploaded=equal&uploaded1=&uploaded2=&disabled_uploads=&invites=equal&invites1=&invites2=&downloaded=equal&downloaded1=&downloaded2=&warned=&invitees=off&invitees1=&invitees2=&snatched=off&snatched1=&snatched2=&comment=&passkey=&avatar=&lastfm=&tracker_ip=&stylesheet=&cc_op=equal&cc=&matchtype=fuzzy&order=Joined&way=Descending&emails_opt=equal&email_cnt=" title="<?= $ip ?>"><?= Lang::get('tools', 'search_ip') ?></a></p>
                </td>

                <td>
                    <input type="text" size="15" name="note" value="<?= base64_decode($note) ?>" title="<?= base64_decode($note) ?>" />
                </td>
                <td>
                    <input type="text" size="15" name="waring" value="<?= base64_decode($waring) ?>" title="<?= base64_decode($waring) ?>" />
                </td>


                <td class="td_btns">
                    <!-- <i class="fa fa-check" aria-hidden="true"> -->
                    <button class="fa fa-check pass" aria-hidden="true" type="submit" name="submit" value="Agree" title="<?= Lang::get('tools', 'pass') ?>" onclick="click_sure('通过')"></button>
                    <!-- </i> -->
                    <button class="fa fa-times refuse" aria-hidden="true" type="submit" name="submit" value="Refuse" title="<?= Lang::get('tools', 'reject') ?>" onclick="click_sure('拒绝')"></button>
                    <button class="fa fa-question pending" aria-hidden="true" type="submit" name="submit" value="Pending" title="<?= Lang::get('tools', 'pend') ?>" onclick="click_sure('待定')"></button>
                    <button class="fa fa-plus add" aria-hidden="true" type="submit" name="submit" value="Add" title="<?= Lang::get('tools', 'add') ?>" onclick="click_sure('补充')"></button>
                </td>

            </form>
        <tr id="detail<?= $ID ?>" style="display:none">
            <td colspan="10" class="application_detail">
