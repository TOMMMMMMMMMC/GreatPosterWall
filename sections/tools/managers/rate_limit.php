...
if ($_POST) {
    authorize();
    if (isset($_POST['task'])) {
        $remove = array_filter($_POST, function ($x) {
            return preg_match('/^remove-\d+$/', $x);
...
<div class="box pad">
    <p>This manager allows you to limit the number of torrent files the
        ...
        <code>overshoot</code> further downloads are allowed per 24 hour rolling window.
    </p>
            ...
            <tr class="row<?= $row ?>">
                ...
                <?php if (check_perms('admin_rate_limit_manage')) { ?>
                    <td>
                        ...
                        <input type="submit" name="remove-<?= $rateLimitInfo['ID'] ?>" value="Remove" onclick="return confirm('Are you sure you want to remove this rate limit? This is an irreversible action!')" />
                    </td>
                <?php } ?>
            </tr>
        <?php
