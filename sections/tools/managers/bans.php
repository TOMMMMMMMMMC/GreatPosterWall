...
View::show_header(Lang::get('tools', 'h2_ip_address_bans'));
$DB->set_query_id($Bans);
?>

<div class="header">
    <h2><?= Lang::get('tools', 'h2_ip_address_bans') ?></h2>
</div>
<div>
    <form class="search_form" name="bans" action="" method="get">
        <table cellpadding="6" cellspacing="1" border="0" class="layout border" width="100%">
            <tr>
                <td class="label"><label for="ip"><?= Lang::get('tools', 'td_ip_address') ?>:</label></td>
                <td>
                    <input type="hidden" name="action" value="ip_ban" />
                    <input type="search" id="ip" name="ip" size="20" value="<?= (!empty($_GET['ip']) ? display_str($_GET['ip']) : '') ?>" />
                </td>
                <td class="label"><label for="notes"><?= Lang::get('tools', 'notes') ?>:</label></td>
                <td>
                    <input type="hidden" name="action" value="ip_ban" />
...
<table width="100%">
    <tr class="colhead">
        <td colspan="2">
            <span class="tooltip" title=""><?= Lang::get('tools', 'range') ?></span>
        </td>
        <td><?= Lang::get('tools', 'notes') ?></td>
