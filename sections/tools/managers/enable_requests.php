...
<div class="header">
    <h2><?= Lang::get('tools', 'auto_enable_requests') ?></h2>
</div>
<div class="linkbox">
    <a class="brackets tooltip" href="tools.php?action=enable_requests" title="<?= Lang::get('tools', 'auto_enable_requests_main_title') ?>"><?= Lang::get('tools', 'auto_enable_requests_main') ?></a>
    <a class="brackets tooltip" href="tools.php?action=enable_requests&amp;view=perfect&amp;<?= Format::get_url(array('view', 'action')) ?>" title="<?= Lang::get('tools', 'auto_enable_requests_perfect_title') ?>"><?= Lang::get('tools', 'auto_enable_requests_perfect') ?></a>
    <a class="brackets tooltip" href="tools.php?action=enable_requests&amp;view=minus_ip&amp;<?= Format::get_url(array('view', 'action')) ?>" title="<?= Lang::get('tools', 'auto_enable_requests_perfect_minus_ip_title') ?>"><?= Lang::get('tools', 'auto_enable_requests_perfect_minus_ip') ?></a>
    <a class="brackets tooltip" href="tools.php?action=enable_requests&amp;view=invalid_email&amp;<?= Format::get_url(array('view', 'action')) ?>" title="<?= Lang::get('tools', 'auto_enable_requests_invalid_email_title') ?>"><?= Lang::get('tools', 'auto_enable_requests_invalid_email') ?></a>
    <a class="brackets tooltip" href="tools.php?action=enable_requests&amp;view=ip_overlap&amp;<?= Format::get_url(array('view', 'action')) ?>" title="<?= Lang::get('tools', 'auto_enable_requests_ip_overlap_title') ?>"><?= Lang::get('tools', 'auto_enable_requests_ip_overlap') ?></a>
    <a class="brackets tooltip" href="tools.php?action=enable_requests&amp;view=manual_disable&amp;<?= Format::get_url(array('view', 'action')) ?>" title="<?= Lang::get('tools', 'auto_enable_requests_manual_disable_title') ?>"><?= Lang::get('tools', 'auto_enable_requests_manual_disable') ?></a>
    <a class="brackets tooltip" href="" title="<?= Lang::get('tools', 'auto_enable_requests_search_title') ?>" onclick="$('#search_form').gtoggle(); return false;"><?= Lang::get('tools', 'auto_enable_requests_search') ?></a>
    <a class="brackets tooltip" href="" title="<?= Lang::get('tools', 'auto_enable_requests_scores_title') ?>" onclick="$('#scores').gtoggle(); return false;"><?= Lang::get('tools', 'auto_enable_requests_scores') ?></a>
</div><br />
<div class="thin">
    <table id="scores" class="hidden" style="width: 50%; margin: 0 auto;">
        <tr>
            <th><?= Lang::get('tools', 'username') ?></th>
            <th><?= Lang::get('tools', 'auto_enable_requests_checked') ?></th>
        </tr>
        <? $DB->query("
    ...
    <form action="" method="GET" id="search_form" <?= !isset($_GET['search']) ? 'class="hidden"' : '' ?>>
        ...
        <table>
            ...
            <tr>
                <td class="label tooltip" title="<?= Lang::get('tools', 'submitted_timestamp_title') ?>"><?= Lang::get('tools', 'submitted_timestamp') ?></td>
                <td>
                    <select name="submitted_between" onchange="ChangeDateSearch(this.value, 'submitted_timestamp2');">
                        <option value="on" <?= $_GET['submitted_between'] == 'on' ? 'selected' : '' ?>><?= Lang::get('tools', 'submitted_on') ?></option>
                        <option value="before" <?= $_GET['submitted_between'] == 'before' ? 'selected' : '' ?>><?= Lang::get('tools', 'submitted_before') ?></option>
                        <option value="after" <?= $_GET['submitted_between'] == 'after' ? 'selected' : '' ?>><?= Lang::get('tools', 'submitted_after') ?></option>
                        <option value="between" <?= $_GET['submitted_between'] == 'between' ? 'selected' : '' ?>><?= Lang::get('tools', 'submitted_between') ?></option>
                    </select>&nbsp;
            ...
            <tr>
                <td class="label tooltip" title="<?= Lang::get('tools', 'handled_timestamp_title') ?>"><?= Lang::get('tools', 'handled_timestamp') ?></td>
                <td>
                    <select name="handled_between" onchange="ChangeDateSearch(this.value, 'handled_timestamp2');">
                        <option value="on" <?= $_GET['handled_between'] == 'on' ? 'selected' : '' ?>><?= Lang::get('tools', 'handled_on') ?></option>
                        <option value="before" <?= $_GET['handled_between'] == 'before' ? 'selected' : '' ?>><?= Lang::get('tools', 'handled_before') ?></option>
                        <option value="after" <?= $_GET['handled_between'] == 'after' ? 'selected' : '' ?>><?= Lang::get('tools', 'handled_after') ?></option>
                        <option value="between" <?= $_GET['handled_between'] == 'between' ? 'selected' : '' ?>><?= Lang::get('tools', 'handled_between') ?></option>
                    </select>&nbsp;
            ...
            <tr>
                ...
                <td>
                    <select name="outcome_search">
                        <option value="">---</option>
                        <option value="<?= AutoEnable::APPROVED ?>" <?= $_GET['outcome_search'] == AutoEnable::APPROVED ? 'selected' : '' ?>><?= Lang::get('tools', 'outcome_approved') ?></option>
                        <option value="<?= AutoEnable::DENIED ?>" <?= $_GET['outcome_search'] == AutoEnable::DENIED ? 'selected' : '' ?>><?= Lang::get('tools', 'outcome_denied') ?></option>
                        <option value="<?= AutoEnable::DISCARDED ?>" <?= $_GET['outcome_search'] == AutoEnable::DISCARDED ? 'selected' : '' ?>><?= Lang::get('tools', 'outcome_discarded') ?></option>
                    </select>
                </td>
            </tr>
            <tr>
                <td class="label"><?= Lang::get('tools', 'include_checked') ?></td>
                <td><input type="checkbox" name="show_checked" <?= isset($_GET['show_checked']) ? Lang::get('tools', 'checked_mark') : '' ?> /></td>
            </tr>
            <tr>
                <td class="label"><?= Lang::get('tools', 'order_by') ?></td>
                <td>
                    <select name="order">
                        <option value="submitted_timestamp" <?= $_GET['order'] == 'submitted_timestamp' ? 'selected' : '' ?>><?= Lang::get('tools', 'submitted_timestamp') ?></option>
                        <option value="outcome" <?= $_GET['order'] == 'outcome' ? 'selected' : '' ?>><?= Lang::get('tools', 'handled_outcome') ?></option>
                        <option value="handled_timestamp" <?= $_GET['order'] == 'handled_timestamp' ? 'selected' : '' ?>><?= Lang::get('tools', 'handled_timestamp') ?></option>
                    </select>&nbsp;
                    <select name="way">
                        <option value="asc" <?= $_GET['way'] == 'asc' ? 'selected' : '' ?>><?= Lang::get('tools', 'ascending') ?></option>
                        <option value="desc" <?= !isset($_GET['way']) || $_GET['way'] == 'desc' ? 'selected' : '' ?>><?= Lang::get('tools', 'descending') ?></option>
                    </select>
                </td>
            </tr>
            <tr>
...
if ($NumResults > 0) { ?>
    ...
    <div class="table_container border">
        <table width="100%" id="enable_requests_table">
            <tr class="colhead">
                <td class="center"><input type="checkbox" id="check_all" /></td>
                <td><?= Lang::get('tools', 'username') ?></td>
                <td><?= Lang::get('tools', 'email_address') ?></td>
                <td><?= Lang::get('tools', 'ip_address') ?></td>
                <td><?= Lang::get('tools', 'user_agent') ?></td>
                <td><?= Lang::get('tools', 'age') ?></td>
                <td><?= Lang::get('tools', 'ban_reason') ?></td>
                <td><?= Lang::get('tools', 'comment') ?><?= $ShowChecked ? Lang::get('tools', 'comment_checked_by') : '' ?></td>
                <td><?= Lang::get('tools', 'submit') ?><?= $ShowChecked ? Lang::get('tools', 'submit_checked_date') : '' ?></td>
                <? if ($ShowChecked) { ?>
                    <td><?= Lang::get('tools', 'outcome') ?></td>
                <?      } ?>
            </tr>
            <?
            $Row = 'a';
