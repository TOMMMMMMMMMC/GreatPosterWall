...
?>
    <div class="header">
        <h2><?= Lang::get('tools', 'vanity_house') ?></h2>
    </div>
    ...
    <div class="thin box pad">
        <form class="create_form" name="album" method="post" action="">
            <div class="pad">
                ...
                <?= Lang::get('tools', 'leave_blank_to_auto_generate') ?>
                <textarea name="Body" cols="95" rows="15"></textarea><br /><br />
                <input type="checkbox" name="FLTorrents" />&nbsp;<?= Lang::get('tools', 'mark_torrents_as') ?>&nbsp;
                <select name="freeleechtype">
                    <option value="1" selected><?= Lang::get('tools', 'fl') ?></option>
                    <option value="2"><?= Lang::get('tools', 'nl') ?></option>
                    <option value="0"><?= Lang::get('tools', 'normal') ?></option>
                </select>
                &nbsp;<?= Lang::get('tools', 'for_reason') ?>&nbsp;<select name="freeleechreason">
                ...
                <input type="checkbox" name="NLOver" />&nbsp;<?= Lang::get('tools', 'nl_torrents_over') ?> <input type="text" name="size" value="<?= isset($_POST['size']) ? $_POST['size'] : '1' ?>" size=1 />
                <select name="scale">
