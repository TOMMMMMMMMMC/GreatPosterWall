...
View::show_header(Lang::get('upload', 'upload'));
...
?>
<div id="dnu_container" class="<?= (check_perms('torrents_hide_dnu') ? 'box pad' : '') ?>">

    <h3 id="dnu_header"><?= Lang::get('upload', 'torrent_diff') ?></h3>
    <p><?= $NewDNU ? '<strong class="important_text">' : '' ?><?= Lang::get('upload', 'last_update') ?>: <?= time_diff($Updated) ?><?= $NewDNU ? '</strong>' : '' ?></p>
    ...
    <div id="dnulist" class="table_container border" class="<?= ($HideDNU ? 'hidden' : '') ?>">
        <table>
            <tr class="colhead">
                <td width="50%"><strong><?= Lang::get('upload', 'name') ?></strong></td>
                <td><strong><?= Lang::get('upload', 'explain') ?></strong></td>
            </tr>
