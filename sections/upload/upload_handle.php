...
$UnsourcedTorrent = $Tor->set_source(); // The source is now GreatPosterWall
...
if ($DB->has_results()) {
    ...
    $DB->query("
		...
		WHERE TorrentID = $ID");
    if ($DB->has_results()) {
        $Err = '<a class="important_text" href="torrents.php?torrentid=' . $ID . '">' . Lang::get('upload', 'same_torrent_exists').'</a>';
    } else {
        // A lost torrent
        $DB->query("
...
if ($Properties['GroupID']) {
    ...
    if ($DB->has_results()) {
        ...
        list($GroupID, $WikiImage, $WikiBody, $RevisionID, $Properties['Name'], $GroupYear, $Properties['ReleaseType'], $Properties['TagList'], $GroupCatalogueNumber, $GroupRecordLabel, $GroupSubName) = $DB->next_record(MYSQLI_NUM, array(4));
    ...
    foreach ($ArtistForm as $Importance => $Artists) {
        foreach ($Artists as $Num => $Artist) {
            ...
            if ($DB->has_results()) {
                list($GroupID, $WikiImage, $WikiBody, $RevisionID, $GroupYear, $GroupCatalogueNumber, $GroupRecordLabel, $GroupSubName) = $DB->next_record();
                if (!$Properties['Image'] && $WikiImage) {
                    $Properties['Image'] = $WikiImage;
                    $T['Image'] = "'" . db_string($WikiImage) . "'";
                }
...
$LogName = Torrents::torrent_group_name($Properties);
...
if ($Type == 'Movies') {
    $Slot = Torrents::CalSlot($Properties);
    $DB->query("
	...
	VALUES
		...
		$T[Remastered], $T[RemasterYear], $T[RemasterTitle],
...
if ($PublicTorrent || $UnsourcedTorrent) {
    View::show_header(Lang::get('upload', 'header_warning'));
?>
...
if ($Properties['ReleaseType'] > 0) {
    $Title .= ' [' . $ReleaseTypes[$Properties['ReleaseType']] . ']';
}
$Details = '';
...
if ($Properties['RemasterTitle']) {
    ...
    foreach ($RemasterTitleNotify as $RTN) {
        $RemasterTitleSQL[] = " RemasterTitles LIKE '%|" . db_string(trim(Lang::get('upload', trim($RTN)))) . "|%' ";
    }
}
