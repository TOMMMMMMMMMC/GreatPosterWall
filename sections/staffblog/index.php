...
?>
    ...
    <div class="box box2 thin">
        <div class="head">
            <?= ((empty($_GET['action'])) ? Lang::get('blog', 'create_a_staff_blog_post') : Lang::get('blog', 'edit_staff_blog_post')) ?>
        ...
        <form class="<?= ((empty($_GET['action'])) ? 'create_form' : 'edit_form') ?>" id="blog_post" name="blog_post" action="staffblog.php" method="post">
            <div id="postform" class="pad<?= (!isset($_REQUEST['action']) || $_REQUEST['action'] != 'editblog') ? ' hidden' : '' ?>">
                ...
                <div class="field_div">
                    ...
                    <input type="text" name="title" size="95" <? if (!empty($Title)) {
                ...
                <div class="field_div">
                    ...
                    <textarea id="quickpost" name="body" cols="95" rows="15"><? if (!empty($Body)) {
                ...
                <div class="submit_div center">
                    ...
                    <input type="submit" value="<?= ((!isset($_GET['action'])) ? Lang::get('blog', 'create_a_blog_post') : Lang::get('blog', 'edit_blog_post')) ?>" />
                </div>
            </div>
        </form>
    </div>
<?
    ...
    View::show_header(Lang::get('blog', 'staff_blog'), 'bbcode');
}
?>
<div class="thin">
    <?
    ...
    ?>
        <div id="blog<?= $BlogID ?>" class="box box2 blog_post">
            <div class="head">
                <strong><?= $Title ?></strong> - <?= Lang::get('blog', 'posted') ?> <?= time_diff($BlogTime); ?> <?= Lang::get('blog', 'by') ?> <a href="user.php?name=<?= $Author ?>"><?= $Author ?></a>
                <? if (check_perms('admin_manage_blog')) { ?>
