...
View::show_header(Lang::get('top10', 'top') . " $Limit " . Lang::get('top10', 'voted_groups'), 'browse,voting');
?>
<div class="thin">
    <div class="header">
        <h2><?= Lang::get('top10', 'top') ?> <?= $Limit ?> <?= Lang::get('top10', 'voted_groups') ?></h2>
    ...
    if (check_perms('site_advanced_top10')) { ?>
        <form class="search_form" name="votes" action="" method="get">
            ...
            <table cellpadding="6" cellspacing="1" border="0" class="layout border top10_votes_filter" width="100%">
                <tr id="tagfilter">
                    <td class="label"><?= Lang::get('top10', 'tags_comma') ?>:</td>
                    <td class="ft_taglist">
                        <input type="text" name="tags" size="75" value="<? if (!empty($_GET['tags'])) {
                                                                            echo display_str($_GET['tags']);
                                                                        } ?>" />&nbsp;
                        <input type="radio" id="rdoAll" name="anyall" value="all" <?= (!isset($TagsAny) ? ' checked="checked"' : '') ?> /><label for="rdoAll">
                            <?= Lang::get('top10', 'all') ?></label>&nbsp;&nbsp;
                        <input type="radio" id="rdoAny" name="anyall" value="any" <?= (isset($TagsAny) ? ' checked="checked"' : '') ?> /><label for="rdoAny"> <?= Lang::get('top10', 'any') ?></label>
                    </td>
                </tr>
                <tr id="yearfilter">
                    <td class="label"><?= Lang::get('top10', 'year') ?>:</td>
                    <td class="ft_year">
                        <input type="text" name="year1" size="4" value="<? if (!empty($_GET['year1'])) {
                                                                            echo display_str($_GET['year1']);
                                                                        } ?>" />
                        <?= Lang::get('top10', 'to') ?>
                        <input type="text" name="year2" size="4" value="<? if (!empty($_GET['year2'])) {
    ...
    <h3><?= Lang::get('top10', 'top') ?> <?= $Limit ?>
        ...
        if (empty($_GET['advanced'])) { ?>
            <small class="top10_quantity_links">
                ...
                switch ($Limit) {
                    case 100: ?>
                        - <a href="top10.php?type=votes" class="brackets"><?= Lang::get('top10', 'top') ?> 25</a>
                        - <span class="brackets"><?= Lang::get('top10', 'top') ?> 100</span>
                        - <a href="top10.php?type=votes&amp;limit=250" class="brackets"><?= Lang::get('top10', 'top') ?> 250</a>
                    <? break;
                    case 250: ?>
                        - <a href="top10.php?type=votes" class="brackets"><?= Lang::get('top10', 'top') ?> 25</a>
                        - <a href="top10.php?type=votes&amp;limit=100" class="brackets"><?= Lang::get('top10', 'top') ?> 100</a>
                        - <span class="brackets"><?= Lang::get('top10', 'top') ?> 250</span>
                    <? break;
                    default: ?>
                        - <span class="brackets"><?= Lang::get('top10', 'top') ?> 25</span>
                        - <a href="top10.php?type=votes&amp;limit=100" class="brackets"><?= Lang::get('top10', 'top') ?> 100</a>
                        - <a href="top10.php?type=votes&amp;limit=250" class="brackets"><?= Lang::get('top10', 'top') ?> 250</a>
                <?    } ?>
            </small>
    ...
    ?>
            <tr class="group discog<?= $SnatchedGroupClass ?>" id="group_<?= $GroupID ?>">
                <td class="center td_collapse">
                    <div id="showimg_<?= $GroupID ?>" class="show_torrents">
                        <a href="#" class="tooltip show_torrents_link" onclick="toggle_group(<?= $GroupID ?>, this, event);" title="<?= Lang::get('global', 'expand_this_group_title') ?>"></a>
                    </div>
                </td>
        ...
        <td colspan="4" class="votes_info_td">
            <span style="white-space: nowrap;">
                <span class="favoritecount_small tooltip"
                    title="<?= $UpVotes . ($UpVotes == 1 ? ' upvote' : ' upvotes') ?>"><span
                        id="upvotes"><?= number_format($UpVotes) ?></span> <span
                        class="vote_album_up"><?=ICONS['vote-up']?></span></span>
                &nbsp; &nbsp;
                <span class="favoritecount_small tooltip"
                    title="<?= $DownVotes . ($DownVotes == 1 ? ' downvote' : ' downvotes') ?>"><span
                        id="downvotes"><?= number_format($DownVotes) ?></span> <span
                        class="vote_album_down"><?=ICONS['vote-down']?></span></span>
                &nbsp;
                <span style="float: right;"><span class="favoritecount_small"
                        id="totalvotes"><?= number_format($TotalVotes) ?></span> <?=Lang::get('top10', 'total')?></span>
            </span>
            <br />
            <span style="white-space: nowrap;">
                <span class="tooltip_interactive"
                    title="&lt;span style=&quot;font-weight: bold;&quot;&gt;Score: <?= number_format($Score * 100, 4) ?>&lt;/span&gt;&lt;br /&gt;&lt;br /&gt;This is the lower bound of the binomial confidence interval &lt;a href=&quot;wiki.php?action=article&amp;id=108&quot;&gt;described here&lt;/a&gt;, multiplied by 100."
                    data-title-plain="Score: <?= number_format($Score * 100, 4) ?>. This is the lower bound of the binomial confidence interval described in the Favorite Album Votes wiki article, multiplied by 100."><?=Lang::get('top10', 'score')?>:
                    <span class="favoritecount_small"><?= number_format($Score * 100, 1) ?></span></span>
                &nbsp; | &nbsp;
                <span class="favoritecount_small"><?= number_format($UpVotes / $TotalVotes * 100, 1) ?>%</span> <?=Lang::get('top10', 'positive')?>
            </span>
        </td>
    </tr>
    <?
            ...
            ?>
                    <tr class="group_torrent groupid_<?= $GroupID ?> edition<?= $SnatchedGroupClass ?> hidden">
                        <td colspan="7" class="edition_info"><strong><a href="#" onclick="torrentTable.toggleEdition(event, <?= $GroupID ?>, <?= $EditionID ?>);" class="tooltip" title="<?= Lang::get('global', 'collapse_this_edition_title') ?>">&minus;</a>
                                <?= Torrents::edition_string($Torrent, $Group) ?></strong></td>
                    </tr>
                <?
                }
                ...
                <tr class="group_torrent torrent_row groupid_<?= $GroupID ?> edition_<?= $EditionID ?><?= $SnatchedTorrentClass . $SnatchedGroupClass ?> hidden">
                    <td colspan="3">
                        <span>
                            ...
                            <?            } ?>
                            | <a href="reportsv2.php?action=report&amp;id=<?= $TorrentID ?>" class="tooltip" title="<?= Lang::get('top10', 'report') ?>">RP</a> ]
                        </span>
                    ...
                    <td class="number_column<?= ($Torrent['Seeders'] == 0) ? ' r00' : '' ?>">
    ...
    <table class="torrent_table grouping cats m_table" id="discog_table">
        <tr class="colhead_dark">
            ...
            <td class="m_th_left" width="70%"><?= Lang::get('global', 'torrents') ?></td>
            <td>
            ...
            <td class="sign snatches">
                ...
                <!-- <img src="static/styles/<?= $LoggedUser['StyleName'] ?>/images/snatched.png" alt="Snatches" title="Snatches" class="tooltip" /> -->
            </td>
            <td class="sign seeders">
                <i class="fa fa-upload tooltip" aria-hidden="true" alt="Seeders" title="<?= Lang::get('global', 'seeders') ?>"></i>
                <!-- <img src="static/styles/<?= $LoggedUser['StyleName'] ?>/images/seeders.png" alt="Seeders" title="Seeders" class="tooltip" /> -->
            </td>
            <td class="sign leechers">
                <i class="fa fa-download tooltip" aria-hidden="true" alt="Leechers" title="<?= Lang::get('global', 'leechers') ?>"></i>
                <!-- <img src="static/styles/<?= $LoggedUser['StyleName'] ?>/images/leechers.png" alt="Leechers" title="Leechers" class="tooltip" /> -->
            </td>
        </tr>
        <?
        if ($TopVotes === false) { ?>
            <tr>
                <td colspan="7" class="center"><?= Lang::get('top10', 'server_is_busy_processing_another_top_list_request') ?></td>
            </tr>
        <?
        } elseif (count($TopVotes) === 0) { ?>
            <tr>
                <td colspan="7" class="center"><?= Lang::get('top10', 'no_torrents_meet_your_criteria') ?></td>
            </tr>
        <?
        } else {
            echo $TorrentTable;
