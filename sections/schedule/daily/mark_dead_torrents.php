...
foreach ($Torrents as $Torrent) {
    list($ID, $GroupID, $Name, $Format, $Encoding, $UserID, $Media, $InfoHash, $Dead) = $Torrent;
    ...
    foreach ($Snatched as $S) {
        if (!array_key_exists($S['uid'], $DeleteNotses)) {
            $DeleteNotes[$S['uid']] = array('Count' => 0, 'Msg' => '');
        }
        $DeleteNotes[$S['uid']]['Msg'] .= "\n[url=" . site_url() . "torrents.php?torrentid=$ID]" . $Name . "[/url]";
        $DeleteNotes[$S['uid']]['Count']++;
    }
