...
while (list($UserID) = $DB->next_record()) {
	...
	Misc::send_pm($UserID, 0, "你已降级 | You have been demoted to " . Users::make_class_string(MEMBER), "你现在只满足 “" . Users::make_class_string(MEMBER) . "” 用户等级的要求。\n\n欲了解更多关于 " . SITE_NAME . " 用户等级的知识，请阅读 [url=" . site_url() . "wiki.php?action=article&amp;name=userclasses]本文[/url]。
...
while (list($UserID) = $DB->next_record()) {
	...
	$Cache->delete_value("user_info_$UserID");
	$Cache->delete_value("user_info_heavy_$UserID");
	Misc::send_pm($UserID, 0, "你已降级 | You have been demoted to " . Users::make_class_string(USER), "你现在只满足 “" . Users::make_class_string(MEMBER) . "” 用户等级的要求。\n\n欲了解更多关于 " . SITE_NAME . " 用户等级的知识，请阅读 [url=" . site_url() . "wiki.php?action=article&amp;name=userclasses]本文[/url]。
