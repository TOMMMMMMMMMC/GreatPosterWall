...
$RequestGroupName = Torrents::torrent_group_name($Data, true);
...
foreach ($UserIDs as $User) {
    list($VoterID) = $User;
    Misc::send_pm($VoterID, 0, "你所寻找的 $FullName 已经发布 | The request $FullName has been filled", '有用户根据你的求种发布了 [url=' . site_url() . "requests.php?action=view&amp;id=$RequestID]$FullName" . '[/url]，你可以点此查看：[url]' . site_url() . "torrents.php?torrentid=$TorrentID" . '[/url]
    
    [hr]
    One of your requests [url=' . site_url() . "requests.php?action=view&amp;id=$RequestID]$FullName" . '[/url] has been filled. You can view it here: [url]' . site_url() . "torrents.php?torrentid=$TorrentID" . '[/url]');
}
