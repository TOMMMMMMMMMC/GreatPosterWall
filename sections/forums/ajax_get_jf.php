...
if (isset($auth_code_arr[4])) {
    ...
    Misc::send_pm($JF_AuthorID, 0, Lang::get('forums', 'award_withdrawal_notification'), Lang::get('forums', 'award_withdrawal_notification_body'));
} else {
    ...
    $Cache->delete_value('user_stats_' . $JF_AuthorID);
    Misc::send_pm($JF_AuthorID, 0, "帖子积分奖励通知 | Post awarded notification", "[url=" . site_url() . "forums.php?action=viewthread&amp;threadid=$JF_TopicID&amp;postid=$JF_PostID#post$JF_PostID]你的帖子[/url] 被奖励了 $JF 积分。
    
    --------------------------------------------------------------------------------
    
    [url=" . site_url() . "forums.php?action=viewthread&amp;threadid=$JF_TopicID&amp;postid=$JF_PostID#post$JF_PostID]Your post[/url] has been awarded $JF bonus points.");
}
