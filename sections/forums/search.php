...
View::show_header(Lang::get('forums', 'forums_greater_than_search'), 'bbcode,forum_search,datetime_picker', 'datetime_picker');
?>
<div class="thin">
    <div class="header">
        <h2><a href="forums.php"><?= Lang::get('forums', 'forums') ?></a> &gt; <?= Lang::get('forums', 'search') ?><?= $Title ?></h2>
    </div>
    <form class="search_form" name="forums" action="" method="get">
        ...
        <table cellpadding="6" cellspacing="1" border="0" class="layout border" width="100%">
            ...
            <tr>
                <td><strong><?= Lang::get('forums', 'posted_by') ?>:</strong></td>
                <td>
                    <input type="search" name="user" placeholder="Username" size="70" value="<?= display_str($User) ?>" />
                </td>
            </tr>
            <tr>
                <td><strong><?= Lang::get('forums', 'topic_created') ?>:</strong></td>
                <td>
            ...
            ?>
                <tr>
                    ...
                    <td>
                        <input type="radio" name="type" id="type_title" value="title" <? if ($Type == 'title') {
                                                                                            echo ' checked="checked"';
                                                                                        } ?> />
                        <label for="type_title"><?= Lang::get('forums', 'titles') ?></label>
                        <input type="radio" name="type" id="type_body" value="body" <? if ($Type == 'body') {
                                                                                        echo ' checked="checked"';
                                                                                    } ?> />
                        <label for="type_body"><?= Lang::get('forums', 'post_bodies') ?></label>
                    </td>
                </tr>
                <tr id="post_created_row" <? if ($Type == 'title') {
                                                echo "class='hidden'";
                                            } ?>>
                    <td><strong><?= Lang::get('forums', 'post_created') ?>:</strong></td>
                    <td>
                        <?= Lang::get('forums', 'after') ?>:
                ...
                <tr>
                    <td><strong><?= Lang::get('forums', 'search_forums') ?>:</strong></td>
                    <td>
            ...
            <?      } ?>
            <td>
                <input type="checkbox" name="forums[]" value="<?= $Forum['ID'] ?>" data-category="forum_category_<?= $i ?>" id="forum_<?= $Forum['ID'] ?>" <? if (isset($_GET['forums']) && in_array($Forum['ID'], $_GET['forums'])) {
                                                                                                                                                                echo ' checked="checked"';
                                                                                                                                                            } ?> />
    ...
    <table cellpadding="6" cellspacing="1" border="0" class="forum_list border" width="100%">
        <tr class="colhead">
            <td><?= Lang::get('forums', 'forum') ?></td>
            <td><?= ((!empty($ThreadID)) ? 'Post begins' : Lang::get('forums', 'topic')) ?></td>
            <td><?= Lang::get('forums', 'topic_creation_time') ?></td>
            <td><?= Lang::get('forums', 'last_post_time') ?></td>
        </tr>
        <? if (!$DB->has_results()) { ?>
            <tr>
                <td colspan="4">Nothing found<?= ((isset($AuthorID) && $AuthorID == 0) ? Lang::get('forums', 'unknown_username') : '') ?>!</td>
            </tr>
        <? }
        ...
        ?>
            <tr class="row<?= $Row ?>">
                ...
                <td>
                    ...
                    if ($Type == 'body') { ?>
                        <a href="#" onclick="$('#post_<?= $PostID ?>_text').gtoggle(); return false;">(Show)</a> <span style="float: right;" class="tooltip last_read" title="Jump to post"><a href="forums.php?action=viewthread&amp;threadid=<?= $ID ?><? if (!empty($PostID)) {
