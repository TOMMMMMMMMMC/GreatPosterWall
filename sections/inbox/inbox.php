...
View::show_header(Lang::get('inbox', 'inbox'));
?>
<div class="thin">
    <h2><?= ($Section === 'sentbox' ? Lang::get('inbox', 'sentbox') : Lang::get('inbox', 'inbox')) ?></h2>
    <div class="linkbox">
        <?
        if ($Section === 'inbox') { ?>
        ...
        <? } else { ?>
            <form class="search_form" name="<?= ($Section === 'sentbox' ? 'sentbox' : 'inbox') ?>" action="inbox.php" method="get" id="searchbox">
                <div>
                    ...
                    <input type="radio" name="searchtype" value="user" <?= (empty($_GET['searchtype']) || $_GET['searchtype'] === 'user' ? ' checked="checked"' : '') ?> /> <?= Lang::get('inbox', 'user') ?>
                    <input type="radio" name="searchtype" value="subject" <?= (!empty($_GET['searchtype']) && $_GET['searchtype'] === 'subject' ? ' checked="checked"' : '') ?> /> <?= Lang::get('inbox', 'subject') ?>
                    <input type="radio" name="searchtype" value="message" <?= (!empty($_GET['searchtype']) && $_GET['searchtype'] === 'message' ? ' checked="checked"' : '') ?> /> <?= Lang::get('inbox', 'message') ?>
                    <span style="float: right;">
                        ...
                        if ($_GET['sort'] === 'unread') { ?>
                            <a href="<?= $BaseURL ?>" class="brackets tooltip" title="<?= $ToggleTitle ?>"><?= Lang::get('inbox', 'list_latest_first') ?></a>
                        <?      } else { ?>
                            <a href="<?= $BaseURL ?>&sort=unread" class="brackets tooltip" title="<?= $ToggleTitle ?>"><?= Lang::get('inbox', 'list_unread_first') ?></a>
                        <?      } ?>
                    </span>
                    <br />
                    <input type="search" name="search" placeholder="<?= (!empty($_GET['search']) ? display_str($_GET['search']) : Lang::get('inbox', 'placeholder_search') . ($Section === 'sentbox' ? Lang::get('inbox', 'sentbox') : Lang::get('inbox', 'inbox'))) ?>" style="width: 98%;" />
                </div>
            </form>
            <form class="manage_form" name="messages" action="inbox.php" method="post" id="messageform">
                ...
                <input type="submit" name="read" value="<?= Lang::get('inbox', 'mark_as_read') ?>" />&nbsp;
                <input type="submit" name="unread" value="<?= Lang::get('inbox', 'mark_as_unread') ?>" />&nbsp;
                <input type="submit" name="delete" value="<?= Lang::get('inbox', 'delete_messages') ?>" />

                <div class="table_container border" id="message_table_container">
                    <table class="message_table checkboxes">
                        <tr class="colhead">
                            <td width="10"><input type="checkbox" onclick="toggleChecks('messageform', this);" /></td>
                            <td width="50%"><?= Lang::get('inbox', 'subject') ?></td>
                            <td><?= ($Section === 'sentbox' ? Lang::get('inbox', 'receiver') : Lang::get('inbox', 'sender')) ?></td>
                            <td><?= Lang::get('inbox', 'date') ?></td>
                            <? if (check_perms('users_mod')) { ?>
                                <td><?= Lang::get('inbox', 'forwarded_to') ?></td>
                            <?      } ?>
                        </tr>
                        <?
                        if ($Count == 0) { ?>
                            <tr class="a">
                                <td colspan="5"><?= Lang::get('inbox', 'no_results') ?></td>
                            </tr>
                            <?  } else {
                            $Row = 'a';
