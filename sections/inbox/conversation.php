...
View::show_header(Lang::get('inbox', 'view_conversation_space') . "$Subject", 'comments,inbox,bbcode,jquery.validate,form_validate');
...
<div class="thin">
    ...
    ?>
    <h3><?= Lang::get('inbox', 'manage_conversation') ?></h3>
    <form class="manage_form" name="messages" action="inbox.php" method="post">
        <div class="box pad">
            ...
            <table class="layout" width="100%">
                <tr>
                    <td class="label"><label for="sticky"><?= Lang::get('inbox', 'sticky') ?></label></td>
                    <td>
                        <input type="checkbox" id="sticky" name="sticky" <? if ($Sticky) {
                                                                                echo ' checked="checked"';
                                                                            } ?> />
                    </td>
                    <td class="label"><label for="mark_unread"><?= Lang::get('inbox', 'mark_as_unread') ?></label></td>
                    <td>
                        <input type="checkbox" id="mark_unread" name="mark_unread" />
                    </td>
                    <td class="label"><label for="delete"><?= Lang::get('inbox', 'delete_conversation') ?></label></td>
                    <td>
    ...
    ?>
        <h3><?= Lang::get('inbox', 'forward_conversation') ?></h3>
