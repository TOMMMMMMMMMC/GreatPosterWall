...
if (isset($_POST['mode'])) {
    switch ($_POST['mode']) {
        case 'update':
            $id   = array_filter($_POST, function ($x) {
                return preg_match('/^id-\d+$/', $x);
            }, ARRAY_FILTER_USE_KEY);
            $ip   = array_filter($_POST, function ($x) {
                return preg_match('/^ip-\d+$/', $x);
            }, ARRAY_FILTER_USE_KEY);
            $name = array_filter($_POST, function ($x) {
                return preg_match('/^name-\d+$/', $x);
            }, ARRAY_FILTER_USE_KEY);
            $sig  = array_filter($_POST, function ($x) {
                return preg_match('/^sig-\d+$/', $x);
            }, ARRAY_FILTER_USE_KEY);
            $ua   = array_filter($_POST, function ($x) {
                return preg_match('/^ua-\d+$/', $x);
            }, ARRAY_FILTER_USE_KEY);
        ...
        case 'remove':
            $rm = array_map(
                ...
                array_keys(
                    array_filter($_POST, function ($x) {
                        return preg_match('/^rm-\d+$/', $x);
                    }, ARRAY_FILTER_USE_KEY)
                )
            );
            $rmid = array_filter($_POST, function ($x) {
                return preg_match('/^rmid-\d+$/', $x);
            }, ARRAY_FILTER_USE_KEY);
            $remove = [];
            foreach ($rm as $id) {
