...
<div class="thin">
    <div class="header">
        <h2><?= Users::format_username($UserID, false, false, false) ?> &gt; <?= Lang::get('badges', 'index_badge') ?> &gt; <?= Lang::get('badges', 'badge_store') ?></h2>
    </div>
    <div class="linkbox">
    ...
    <div class="badge_store_container">
        <table id="badge_store_table">
            <tr class="colhead">
                <td class="badge_number">#</td>
                <td class="badge_preview"><?= Lang::get('badges', 'badge_preview') ?></td>
                <td class="badge_name"><?= Lang::get('badges', 'badge_name') ?></td>
                <td class="badge_introduction"><?= Lang::get('badges', 'badge_introduction') ?></td>
                <td class="badge_unshelve"><?= Lang::get('badges', 'badge_unshelve') ?></td>
                <td class="badge_price"><?= Lang::get('badges', 'badge_price') ?></td>
                <td class="badge_exchange"><?= Lang::get('badges', 'badge_exchange') ?></td>
                <!-- 每一列都要加这些 class -->
            </tr>
            <?
            $i = 0;
            ...
            ?>
                    <tr class="row<?= $i % 2 ? "a" : "b" ?>">
                        <td class="badge_number"><?= $i ?></td>
                        <td class="badge_preview">
                            <img src="<?= $Badge['BigImage'] ?>">
                        </td>
                        <td class="badge_name"><?= Badges::get_text($Label, 'badge_name') ?></td>
                        <td class="badge_introduction"><?= Badges::get_text($Label, 'badge_introduction') ?></td>
                        <td class="badge_unshelve"><?= time_diff($Items[$Badge['Level']]['unshelve']) ?></td>
                        <td class="badge_price"><?= $Items[$Badge['Level']]['price'] ?></td>
                        <td class="badge_exchange">
                            <?=
                            $Exchanged ?
                                "Exchanged" : ($Unshelved ?
