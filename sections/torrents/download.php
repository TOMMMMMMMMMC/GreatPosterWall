...
if (!is_array($Info) || !array_key_exists('PlainArtists', $Info) || empty($Info[10])) {
    ...
    $Info = array($DB->next_record(MYSQLI_NUM, array(6, 7, 8, 12)));
    ...
    $Info['Sitename'] = '[Greatposterwall]  ';
    $Cache->cache_value("torrent_download_$TorrentID", $Info, 0);
}
if (!is_array($Info[0])) {
    error(404);
}
list($Source, $Codec, $Container, $Resolution, $Makers, $Year, $GroupID, $Name, $Image, $CategoryID, $Size, $FreeTorrent, $InfoHash, $TorrentUploaderID, $SubName, $Year) = array_shift($Info); // used for generating the filename
...
if ($_REQUEST['usetoken'] && in_array($FreeTorrent, ['0', '11', '12', '13'])) {
    if (isset($LoggedUser)) {
...
$FileName = TorrentsDL::construct_file_name($Info['PlainArtists'], $Name, $Source, $Codec, $Container, $Resolution, $Makers, $Year);
...
header('Content-disposition: attachment; filename="[GPW] ' . $FileName . '"');
