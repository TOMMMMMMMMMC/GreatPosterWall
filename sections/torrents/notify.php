...
if ($Sneaky) {
    ...
    View::show_header($UserInfo['Username'] . Lang::get('torrents', 's_notifications'), Lang::get('torrents', 'notifications'));
} else {
    View::show_header(Lang::get('torrents', 'my_notifications'), Lang::get('torrents', 'notifications'));
}
?>
<div class="thin widethin">
    <div class="header">
        <h2><?= Lang::get('torrents', 'latest_notifications') ?></h2>
    </div>
    <div class="linkbox">
        <? if ($FilterID) { ?>
        ...
        ?>
            ...
            <form class="torrent-table" name="torrents" id="notificationform_<?= $FilterID ?>" action="">
                <table class="torrent_table cats checkboxes border m_table">
                    <?
                    print_torrent_table_header(TorrentTableScene::Notify, ['FilterID' => $FilterID]);
                    ...
                    foreach ($FilterResults as $Result) {
                        ...
                        print_ungroup_info($TorrentInfo, $GroupInfo, $GroupCategoryID, $TorrentTags, $Director, 'action=basic', null, $FilterID);
                    }
                    ?>
                </table>
            </form>
