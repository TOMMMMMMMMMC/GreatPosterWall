...
$DBTorVals = array();
$DB->query("
	SELECT Media, Format, Encoding, Source, Codec, Container, Resolution, Subtitles, Makers , Scene, Jinzhuan, Diy, Buy, Allow, Description, FileList, RemasterTitle, RemasterCustomTitle, Processing, NotMainMovie, SpecialSub, ChineseDubbed, MediaInfo, Note, SubtitleType, RemasterYear
	FROM torrents
	WHERE ID = $TorrentID");
...
if (check_perms('users_mod')) {
    ...
    $DB->query("
		SELECT TorrentID
		FROM torrents_bad_img
		WHERE TorrentID = '$TorrentID'");
    list($biID) = $DB->next_record();

    if (!$biID && $Properties['BadImg']) {
        $DB->query("
			INSERT INTO torrents_bad_img
			VALUES ($TorrentID, $LoggedUser[ID], '" . sqltime() . "')");
    }
    ...
    $DB->query("
		SELECT TorrentID
		FROM torrents_bad_compress
		WHERE TorrentID = '$TorrentID'");
    list($tbcID) = $DB->next_record();

    if (!$tbcID && $Properties['BadCompress']) {
        $DB->query("
			INSERT INTO torrents_bad_compress
			VALUES ($TorrentID, $LoggedUser[ID], '" . sqltime() . "')");
    }
    if ($tbcID && !$Properties['BadCompress']) {
        $DB->query("
