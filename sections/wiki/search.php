...
View::show_header(Lang::get('wiki', 'search_articles'));
$DB->set_query_id($RS);
?>
<div class="thin">
    <div class="header">
        <h2><?= Lang::get('wiki', 'search_articles') ?></h2>
        <div class="linkbox">
                    ...
                    <td>
                        <select name="order">
                            <? foreach ($OrderVals as $Cur) { ?>
                                <option value="<?= $Cur ?>" <? if ($_GET['order'] == $Cur || (!$_GET['order'] && $Cur == 'Time')) {
                        ...
                        <select name="way">
                            <? foreach ($WayVals as $Cur) { ?>
                                <option value="<?= $Cur ?>" <? if ($_GET['way'] == $Cur || (!$_GET['way'] && $Cur == 'Descending')) {
                                                                echo ' selected="selected"';
                                                            } ?>><?= $Cur ?></option>
                            <?                  } ?>
                        </select>
    ...
    <?  } ?>
    <div class="table_container border wiki_article_table">
        <table width="100%">
            <tr class="colhead">
                <td><?= Lang::get('wiki', 'article') ?></td>
                <td><?= Lang::get('wiki', 'last_updated_on') ?></td>
