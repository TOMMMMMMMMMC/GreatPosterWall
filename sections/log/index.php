...
View::show_header(Lang::get('log', 'site_log'));

include(SERVER_ROOT . '/sections/log/sphinx.php');
?>
<div class="thin">
    <div class="header">
        <h2><?= Lang::get('log', 'site_log') ?></h2>
    </div>
    <div class="box pad">
        <form class="search_form" name="log" action="" method="get">
            <table cellpadding="6" cellspacing="1" border="0" class="layout border" width="100%">
                <tr>
                    <td class="label"><strong><?= Lang::get('log', 'search_for') ?>:</strong></td>
                    <td>
    ...
    <div class="table_container border">
        <table cellpadding="6" cellspacing="1" border="0" class="log_table border" id="log_table" width="100%">
            <tr class="colhead">
                <td style="width: 180px;"><strong><?= Lang::get('log', 'time') ?></strong></td>
                <td><strong><?= Lang::get('log', 'message') ?></strong></td>
            </tr>
            <? if ($QueryStatus) { ?>
                <tr class="nobr">
                    <td colspan="2"><?= Lang::get('log', 'search_request_failed') ?> (<?= $QueryError ?>)<?= Lang::get('log', 'period') ?></td>
                </tr>
            <?  } elseif (!$DB->has_results()) { ?>
                <tr class="nobr">
                    <td colspan="2"><?= Lang::get('log', 'nothing_found') ?></td>
                </tr>
                <?
            }
            $Row = 'a';
