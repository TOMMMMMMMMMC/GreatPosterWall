...
if ($Data) {
    list($K, list($Name, $Image, $Body, $IMDBID, $NumSimilar, $SimilarArray,,, $VanityHouseArtist)) = each($Data);
} else {
    if ($RevisionID) {
        $sql = "
			SELECT
    ...
    list($Name, $Image, $Body, $IMDBID, $VanityHouseArtist) = $DB->next_record(MYSQLI_NUM, array(0));
}
...
$Data = array(array($Name, $Image, $Body, $IMDBID, $NumSimilar, $SimilarArray, array(), array(), $VanityHouseArtist));
