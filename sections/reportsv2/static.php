...
View::show_header(Lang::get('reportsv2', 'reports_v2'), 'reportsv2,bbcode');
?>
<div class="header">
    <h2><?= $Title ?></h2>
    <? include('header.php'); ?>
    ...
    ?>
        ...
        foreach ($Reports as $Report) {
            ...
            list(
                ...
                $RemasterYear, $Codec, $Source, $Resolution, $Container, $Size, $HasLog, $HasCue, $HasLogDB, $LogScore, $LogChecksum, $UploaderID, $UploaderName
        ...
        ?>
                <div id="report<?= $ReportID ?>" class="report box pad center">
                    <a href="reportsv2.php?view=report&amp;id=<?= $ReportID ?>"><?= Lang::get('reportsv2', 'report') ?> <?= $ReportID ?></a> <?= Lang::get('reportsv2', 'report_for_torrent') ?> <?= $TorrentID ?> <?= Lang::get('reportsv2', 'deleted_has_been_automatically_resolved') ?><input type="button" value="Hide" onclick="ClearReport(<?= $ReportID ?>);" />
                </div>
            <?
            } else {
                if (!$CategoryID) {
                ...
                $RemasterDisplayArray = [];
                if ($RemasterTitle) {
                    $RemasterTitleArray = explode(' / ', $RemasterTitle);
                    foreach ($RemasterTitleArray as $R) {
                        $RemasterDisplayArray[] = Lang::get('upload', $R);
                    }
                    $RemasterDisplayString = implode('/', $RemasterDisplayArray);
                }
                $RawName = Torrents::torrent_group_name($Report);
...
*/
                        ...
                        <div class="table_container border">
                            <table class="box layout" cellpadding="5">
                                <tr>
                                    <td class="label"><a href="reportsv2.php?view=report&amp;id=<?= $ReportID ?>"><?= Lang::get('reportsv2', 'reported_torrent_1') ?></a> <?= Lang::get('reportsv2', 'reported_torrent_2') ?>:</td>
                                    <td colspan="3">
                                        <? if (!$GroupID) { ?>
                                            <a href="log.php?search=Torrent+<?= $TorrentID ?>"><?= $TorrentID ?></a> (<?= Lang::get('reportsv2', 'deleted') ?>)
                                        <?          } else { ?>
                                            <?= $LinkName ?>
                                            ...
                                            <?= Lang::get('reportsv2', 'uploaded_by_sb_on_st_1') ?> <a href="user.php?id=<?= $UploaderID ?>"><?= $UploaderName ?></a> <?= Lang::get('reportsv2', 'uploaded_by_sb_on_st_2') ?> <span title="<?= time_diff($Time, 3, false) ?>"><?= $Time ?></span>
                                            <br />
                                            <? if ($ReporterName == '') {
                                                $ReporterName = 'System';
                                            } ?>
                                            <div style="text-align: right;"><?= Lang::get('reportsv2', 'was_reported_by_sb_st_for_the_reason_1') ?> <a href="user.php?id=<?= $ReporterID ?>"><?= $ReporterName ?></a> <?= Lang::get('reportsv2', 'was_reported_by_sb_st_for_the_reason_2') ?> <?= time_diff($ReportedTime) ?> <?= Lang::get('reportsv2', 'was_reported_by_sb_st_for_the_reason_3') ?>: <strong><?= $ReportType['title'] ?></strong></div>
                                            <? if ($Status != 'Resolved') {
						...
						WHERE rep.Status != 'Resolved'
							...
							AND req.TorrentID = $TorrentID");
                                                ...
                                                if ($Requests > 0) {
                                                    ...
                                                    ?>
                                                        <div style="text-align: right;">
                                                            <strong class="important_text"><a href="user.php?id=<?= $FillerID ?>"><?= $FillerName ?></a> <?= Lang::get('reportsv2', 'used_this_torrent_to_fill') ?> <a href="requests.php?action=view&amp;id=<?= $RequestID ?>"><?= Lang::get('reportsv2', 'this_request') ?></a> <?= time_diff($FilledTime) ?></strong>
                                                        </div>
                                        <?                      }
                                                }
                                            }
                                ...
                                <? if ($Tracks) { ?>
                                    <tr>
                                        <td class="label"><?= Lang::get('reportsv2', 'relevant_tracks') ?>:</td>
                                        <td colspan="3">
                                            <?= str_replace(' ', ', ', $Tracks) ?>
                                        </td>
                                    </tr>
                                ...
                                if ($Links) { ?>
                                    <tr>
                                        <td class="label"><?= Lang::get('reportsv2', 'relevant_links') ?>:</td>
                                        <td colspan="3">
                                            <?
                                            $Links = explode(' ', $Links);
                                ...
                                if ($ExtraIDs) { ?>
                                    <tr>
                                        <td class="label"><?= Lang::get('reportsv2', 'relevant_other_torrents') ?>:</td>
                                        <td colspan="3">
                                            <?
                                            $First = true;
						...
						WHERE t.ID = '$ExtraID'
						GROUP BY tg.ID");
                                                $RawData = $DB->next_record();

                                                list(
                                                    $ExtraGroupName, $ExtraGroupID, $ExtraArtistID, $ExtraArtistName, $ExtraYear, $ExtraTime, $ExtraRemastered, $ExtraRemasterTitle,
                                                    $ExtraRemasterYear, $ExtraMedia, $ExtraFormat, $ExtraEncoding, $ExtraSize, $ExtraHasCue, $ExtraHasLog, $ExtraLogScore, $ExtraUploaderID, $ExtraUploaderName
                                                ) = Misc::display_array($RawData);

                                                if ($ExtraGroupName) {
                                                    $ExtraName = Torrents::torrent_group_name($RawData);
                                                    $ExtraLinkName = "<a href='torrents.php?torrentid=$ExtraID#$ExtraID'>$ExtraName</a>";
                                            ?>
                                                    <?= ($First ? '' : '<br />') ?>
                                                    <?= $ExtraLinkName ?>
                                                    <a href="torrents.php?action=download&amp;id=<?= $ExtraID ?>&amp;authkey=<?= $LoggedUser['AuthKey'] ?>&amp;torrent_pass=<?= $LoggedUser['torrent_pass'] ?>" title="<?= Lang::get('global', 'download') ?>" class="brackets tooltip">DL</a>
                                                    <?= Lang::get('reportsv2', 'uploaded_by_sb_on_st_1') ?> <a href="user.php?id=<?= $ExtraUploaderID ?>"><?= $ExtraUploaderName ?></a> <?= Lang::get('reportsv2', 'uploaded_by_sb_on_st_2') ?> <span title="<?= time_diff($ExtraTime, 3, false) ?>"><?= $ExtraTime ?></span>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="label"><?= Lang::get('reportsv2', 'switch') ?>:</td>
                                        <td colspan="3"><a href="#" onclick="Switch(<?= $ReportID ?>, <?= $TorrentID ?>, <?= $ExtraID ?>); return false;" class="brackets"><?= Lang::get('reportsv2', 'switch') ?></a> <?= Lang::get('reportsv2', 'the_source_and_target_torrents') ?>
                                    <?
                                                    $First = false;
                                                }
                                            }
                                ...
                                ?>
                                    <tr>
                                        <td class="label"><?= Lang::get('reportsv2', 'relevant_images') ?>:</td>
                                        <td colspan="3">
                                            <?
				...
				and LastPostID = fp.id"
                                    ...
                                    //[quote=Comments]两位艺术家应该分开，已经单独添加，申请删除合并[/quote]
                                    if ($DB->has_results()) {
                                        $EditTopics = $DB->to_array();
                                    ?>
                                        <tr>
                                            <td class="label"><?= Lang::get('reportsv2', 'edit_request_about_this_torrent') ?>:</td>
                                            <td colspan="3" class="wrap_overflow">
                                                <table>
                                                    <?
                                ...
                                ?>
                                <tr>
                                    <td class="label"><?= Lang::get('reportsv2', 'user_comment') ?>:</td>
                                ...
                                ?>
                                    <tr>
                                        <td class="label"><?= Lang::get('reportsv2', 'uploader_s_reply') ?>:</td>
                                        <td colspan="3" class="wrap_overflow"><?= Text::full_format($UploaderReply) ?></td>
                                    </tr>
                                <?
                                }
                                ?>
                                ...
                                if ($Status == 'InProgress') { ?>
                                    <tr>
                                        <td class="label"><?= Lang::get('reportsv2', 'in_progress_by') ?>:</td>
                                        <td colspan="3">
                                ...
                                if ($Status != 'Resolved') { ?>
                                    <tr>
                                        <td class="label"><?= Lang::get('reportsv2', 'report_comment') ?>:</td>
                                        <td colspan="3">
                                    ...
                                    <tr>
                                        <td class="label">
                                            <a href="javascript:Load('<?= $ReportID ?>')" class="tooltip" title="<?= Lang::get('reportsv2', 'resolve_title') ?>"><?= Lang::get('reportsv2', 'resolve') ?></a>:
                                        </td>
                                        <td colspan="3">
                                            ...
                                            | <span id="options<?= $ReportID ?>">
                                                <span class="tooltip" title="<?= Lang::get('reportsv2', 'warning_title') ?>">
                                                ...
                                                if ($FirstTorrent) {
                                                ?>
                                                    <span class="tooltip" title="<?= Lang::get('reportsv2', 'first_torrent_title') ?>"><strong class="important_text"><?= Lang::get('reportsv2', 'first_torrent') ?></strong></span> |
                                                <?
                                                }
                                                ?>
                                                <? if (check_perms('users_mod')) { ?>
                                                    <span class="tooltip" title="<?= Lang::get('reportsv2', 'delete_title') ?>">
                                                        <input type="checkbox" name="delete" id="delete<?= $ReportID ?>" />&nbsp;<label for="delete<?= $ReportID ?>"><strong><?= Lang::get('global', 'delete') ?></strong></label>
                                                    </span> |
                                                <?              } ?>
                                                <span class="tooltip" title="<?= Lang::get('reportsv2', 'remove_upload_privileges_title') ?>">
                                                ...
                                                <span class="tooltip" title="<?= Lang::get('reportsv2', 'update_now_title') ?>">
                                    ...
                                    <tr>
                                        <td class="label tooltip"><?= Lang::get('reportsv2', 'custom_trumpable') ?>:</td>
                                        <td colspan="3">
                                            <textarea name="custom_trumpable" id="custom_trumpable<?= $ReportID ?>" cols="50" rows="2" style="min-height: auto;"></textarea>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="label tooltip" title="<?= Lang::get('reportsv2', 'pm_uploader_reporter_title') ?>">
                                            <?= Lang::get('reportsv2', 'pm_uploader_reporter') ?>
                                            <select name="pm_type" id="pm_type<?= $ReportID ?>">
                                                <option value="Uploader" selected="selected"><?= Lang::get('reportsv2', 'uploader') ?></option>
                                                <option value="Reporter"><?= Lang::get('reportsv2', 'reporter') ?></option>
                                            </select>:
                                        </td>
                                        <td colspan="3">
                                    ...
                                    <tr>
                                        <td class="label"><strong><?= Lang::get('reportsv2', 'extra') ?></strong><?= Lang::get('reportsv2', 'space_log_message') ?>:</td>
                                        <td>
                                        ...
                                        <td class="label"><strong><?= Lang::get('reportsv2', 'extra') ?></strong><?= Lang::get('reportsv2', 'space_staff_notes') ?>:</td>
                                        <td>
                                    ...
                                    <tr>
                                        <td colspan="4" style="text-align: center;">
                                            ...
                                            <?                  }   ?>
                                            | <span class="tooltip" title="<?= Lang::get('reportsv2', 'multi_resolve_title') ?>">
                                ...
                                <?              } else { ?>
                                    <tr>
                                        <td class="label"><?= Lang::get('reportsv2', 'resolver') ?>:</td>
                                        <td colspan="3">
                                            <a href="user.php?id=<?= $ResolverID ?>"><?= $ResolverName ?></a>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="label"><?= Lang::get('reportsv2', 'resolve_time') ?>:</td>
                                        <td colspan="3">
                                    ...
                                    <tr>
                                        <td class="label"><?= Lang::get('reportsv2', 'log_message') ?>:</td>
                                        <td colspan="3">
                                            <?= $LogMessage;
                                            echo "\n"; ?>
                                        </td>
