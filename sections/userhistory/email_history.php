...
<div class="table_container border">
	<table width="100%" id="email_history_table">
		<tr class="colhead">
			<td><?= Lang::get('userhistory', 'email') ?></td>
			<td><?= Lang::get('userhistory', 'set') ?></td>
			<td>IP <a href="userhistory.php?action=ips&amp;userid=<?= $UserID ?>" class="brackets">H</a></td>
			<? if ($UsersOnly == 1) {
			?>
				<td><?= Lang::get('userhistory', 'user') ?></td>
			<?
			}
			?>
		</tr>
		...
		?>
			<tr class="rowa">
				...
				<td><?= display_str($Values['IP']) ?> (<?= display_str($Values['Code']) ?>) <a href="user.php?action=search&amp;ip_history=on&amp;ip=<?= display_str($Values['IP']) ?>" class="brackets tooltip" title="<?= Lang::get('userhistory', 'search') ?>">S</a></td>
				<?
				if ($UsersOnly == 1) {
					$ueQuery = $DB->query("
					SELECT
