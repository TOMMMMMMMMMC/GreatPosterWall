...
View::show_header(Lang::get('userhistory', 'post_history_for_before') . "$Username" . Lang::get('userhistory', 'post_history_for_after'), 'subscriptions,comments,bbcode');
                    ...
                    <tr class="colhead_dark">
                        <td colspan="<?= Users::has_avatars_enabled() ? 2 : 1 ?>">
                            <span style="float: left;">
                                <?= time_diff($AddedTime) ?><?= Lang::get('userhistory', 'in') ?><a href="forums.php?action=viewthread&amp;threadid=<?= $TopicID ?>&amp;postid=<?= $PostID ?>#post<?= $PostID ?>" class="tooltip" title="<?= display_str($ThreadTitle) ?>"><?= Format::cut_string($ThreadTitle, 75) ?></a>
                                <?
                                if ($ViewingOwn) {
                                    if ((!$Locked || $Sticky) && (!$LastRead || $LastRead < $LastPostID)) { ?>
                                        <span class="new">(<?= Lang::get('userhistory', 'new') ?>!)</span>
                                    <?
                                    }
                                    ?>
                            </span>
                            <? if (!empty($LastRead)) { ?>
                                <span style="float: left;" class="tooltip last_read" title="<?= Lang::get('global', 'jump_to_last_read') ?>">
