...
class NotificationsManager {
    ...
    public function load_news() {
        ...
        if ($CurrentNews === false || $Title === false) {
            $QueryID = G::$DB->get_query_id();
            G::$DB->query("SELECT `ID`, `Title` FROM `forums_topics` WHERE `ForumID` = '7' AND  `IsNotice` = '1'  ORDER BY `IsSticky` DESC, `CreatedTime` DESC LIMIT 1");
            if (G::$DB->has_results()) {
    ...
    public static function clear_news($News = false) {
        ...
        if (!$News) {
            if (!$News = G::$Cache->get_value('news')) {
                G::$DB->query(
                    "SELECT `ID`, `Title`, 'NULL', `CreatedTime` FROM `forums_topics` WHERE `ForumID` = '7' AND  `IsNotice` = '1'  ORDER BY `IsSticky` DESC, `CreatedTime` DESC LIMIT 1"
                );
