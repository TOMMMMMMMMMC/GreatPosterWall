...
class Format {
    ...
    private static $TorrentLabels = array(
        ...
        'Missing Lineage' => 'tl_trumpable tl_missing_lineage',
    ...
    public static function get_ratio_html($Dividend, $Divisor, $Color = true, $Tooltip = true) {
        ...
        if ($Ratio === '∞') {
            ...
            return "<span class='$Class r99' title='$Title'>∞</span>";
        }
        if ($Color) {
            $Ratio = sprintf(
    ...
    public static function torrent_label($Text, $Class = '', $Title = '') {
        ...
        if (!empty($Title)) {
            return $Icon . sprintf(
                '<span class="torrent_label tooltip %1$s" title="%2$s" style="white-space: nowrap;">%3$s</span>',
                display_str($Class),
                display_str($title),
                display_str($Text)
            );
            ...
            return $Icon . sprintf(
                '<span class="torrent_label %1$s" style="white-space: nowrap;">%2$s</span>',
                display_str($Class),
                display_str($Text)
            );
        }
