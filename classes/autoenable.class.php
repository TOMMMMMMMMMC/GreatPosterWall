...
class AutoEnable {
    ...
    const RECEIVED_MESSAGE = "Your request to re-enable your account has been received. You can expect a reply message in your email within 48 hours.<br />If you do not receive an email after 48 hours have passed, please visit us on <a href='https://t.me/joinchat/KNqyAh4_iO9jY2U1'>Telegram</a> for assistance.";
    ...
    public static function handle_requests($IDs, $Status, $Comment) {
        ...
        foreach ($UserInfo as $User) {
            list($ID, $UserID) = $User;
            $BaseComment = sqltime() . Lang::get('login', 'enable_request_id_by_user_before') . "$ID " . strtolower(self::get_outcome_string($Status)) . Lang::get('login', 'enable_request_id_by_user_after') . '[user]' . $StaffUser . '[/user]';
