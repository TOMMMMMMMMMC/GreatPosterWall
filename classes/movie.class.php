...
class MOVIE {
    ...
    public static function get_imdb_actor_data($IMDBID, $Refresh = false) {
        ...
        if (!$IMDBActorData || $Refresh) {
            $IMDBResult = new \Imdb\Title($IMDBID);
            $IMDBActorInfo = [];
    ...
    public static function get_movie($IMDBID, $Refresh = false) {
        $OMDBData = null;
        $TMDBData = null;
        ...
        if (empty($TMDBData) || $Refresh) {
            $tmdb = $multi_curl->addGet('https://api.themoviedb.org/3/find/' . $IMDBID, ['api_key' => $key, 'language' => 'zh-CN', 'external_source' => 'imdb_id']);
            $tmdb->myTag = 'tmdb';
        }
        if (empty($DoubanActorData) || $Refresh) {
        ...
        if ($OMDBData) {
            ...
            $Info['ReleaseDate'] = $OMDBResult->Released;
            $Info['Year'] = $OMDBResult->Year;
            $Info['Runtime'] = $OMDBResult->Runtime;
            $Info['IMDBRating'] = $OMDBResult->imdbRating;
            $Info['Region'] = $OMDBResult->Country;
            $Info['Language'] = $OMDBResult->Language;
        ...
        $IMDBResult = new \Imdb\Title($IMDBID);
        $Info['Title'] = $IMDBResult->title();
        $Info['Plot'] = $IMDBResult->plot();
        $Info['Year'] = $IMDBResult->year();
        $Info['Genre'] = strtolower(implode(',', $IMDBResult->genres()));
        $Info['IMDBRating'] = $IMDBResult->rating();
        $Info['ReleaseDate'] = $IMDBResult->releaseInfo();
        $Info['Language'] = $IMDBResult->languages();
        $Info['Runtime'] = $IMDBResult->runtime();
        ...
        if ($IMDBActorData) {
            ...
            if ($DoubanActorData) {
                ...
                if ($DoubanResult->data) {
                    ...
                    if ($Info['Region'] && (str_contains($Info['Region'], 'China') || str_contains($Info['Region'], 'Hong Kong') || str_contains($Info['Region'], 'Taiwan') || str_contains($Info['Region'], 'Japan') || str_contains($Info['Region'], 'South Korea')) && $DoubanResult->data->actors) {
    ...
    public static function upload_movie_poster($IMDBID, $url) {
        return self::upload_image('site/movie/imdb/' . $IMDBID . '.jpg', $url);
    }
    public static function upload_artist_avatar($IMDBID, $url) {
        return self::upload_image("site/artist/imdb/" . $IMDBID . ".jpg", $url);
    }

    private static function upload_image($key, $url) {
        global $Minio;
        try {
            $Addr = $Minio->fetch_upload($key, $url);
        } catch (Exception $e) {
            error_log($e);
            return "";
        }
