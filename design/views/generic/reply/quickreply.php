    ...
    <div class="box pad">
            ...
            <tr class="colhead_dark">
                <td colspan="<?= (Users::has_avatars_enabled() ? 2 : 1) ?>">
                    <div style="float: left;"><a href="#quickreplypreview">#XXXXXX</a>
                        <?= Lang::get('forums', 'by_user_just_now_before') ?><strong><?= Users::format_username(G::$LoggedUser['ID'], true, true, true, true) ?></strong><?= Lang::get('forums', 'by_user_just_now_after') ?>
                    </div>
                    <div style="float: right;">
                        <a href="#quickreplypreview" class="brackets"><?= Lang::get('forums', 'report') ?></a>
                        &nbsp;
                        <a href="#">&uarr;</a>
                    </div>
                </td>
            ...
            <tr>
                <? if (Users::has_avatars_enabled()) { ?>
                    <td class="avatar" valign="top">
                        <?= Users::show_avatar(G::$LoggedUser['Avatar'], G::$LoggedUser['ID'], G::$LoggedUser['Username'], isset($HeavyInfo['DisableAvatars']) ? $HeavyInfo['DisableAvatars'] : 0) ?>
                    </td>
                <?  } ?>
        ...
        <form class="send_form center" name="reply" id="quickpostform" action="<?= $Action ?>" method="post" <? if (!check_perms('users_mod')) { ?> onsubmit="quickpostform.submit_button.disabled = true;" <? } ?>>
            ...
            <div class="preview_submit">
                ...
                ?>
                <input type="button" value="<?= Lang::get('upload', 'preview') ?>" class="button_preview_<?= $ReplyText->getID() ?> bbcode-preview-button" data-id="<?= $ReplyText->getID() ?>" tabindex="1" />
