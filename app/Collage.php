...
class Collage extends BaseObject {
    ...
    protected function loadArtists() {
        ...
        uasort($this->artists, function ($x, $y) {
    ...
    public function maxGroupsPerUser() {
        return $this->maxGroupsPerUser;
    }
    public function name() {
        return $this->name;
    }
    public function numArtists() {
        return count($this->artists);
    }
    public function numContributors() {
    ...
    public function isArtist(): bool {
        return $this->categoryId === COLLAGE_ARTISTS_ID;
    }
    ...
    public function isLocked(): bool {
        return $this->locked == '1' || (isset($this->lockedForUser) && $this->lockedForUser);
    }
    ...
    public function remove(User $user, Manager\Subscription $subMan, Log $logger, string $reason): int {
        ...
        $this->flush();
        $logger->general(sprintf(
            "Collage %d (%s) was deleted by %s: %s",
            $this->id,
            $this->name,
            $user->name,
            $reason
