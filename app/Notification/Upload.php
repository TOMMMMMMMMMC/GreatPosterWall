...
class Upload extends \Gazelle\Base {
    ...
    public function addFormatBitrate(array $fbList) {
        $newFormat  = array_merge(array_map(function ($f) {
            return $f['format'];
        }, $fbList), ['']);
        $newBitrate = array_merge(array_map(function ($f) {
            return $f['bitrate'];
        }, $fbList), ['']);
    ...
    public function trigger(int $groupId, int $torrentId, \Feed $feed, string $item): int {
        ...
        if ($this->debug) {
            $this->debug->set_flag(sprintf(
                'upload: notification complete: (%d users)',
                count($results)
            ));
        }
