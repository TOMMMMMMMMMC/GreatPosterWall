...
class Bonus {
    ...
    public function sendPmToOther($from, $toID, $amount) {
        $ToUserLang = \Lang::getUserLang($toID);
        if ($amount > 1) {
            $is_are = 'are';
            $s = 's';
        } else {
		...
		{$from} 向你赠送了 {$amount} 枚免费令牌供你消费！" .
            "你可在下载种子时使用它们，从而避免下载量的增长。" .
            "关于免费令牌的更多信息请见 " .
            "[url=" . site_url() . "wiki.php?action=article&name=免费令牌]这篇 wiki[/url]。
...
Enjoy!";
        \Misc::send_pm($toID, 0, "你收到了 {$amount} 枚免费令牌！| Here {$is_are} {$amount} freeleech token{$s}!", trim($Body));
    }
