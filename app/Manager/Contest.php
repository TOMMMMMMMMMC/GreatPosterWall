...
class Contest extends \Gazelle\Base {
    ...
    public function create(array $info) {
        ...
        if (isset($info['pool'])) {
            $this->db->prepared_query(
                "INSERT INTO bonus_pool (name, since_date, until_date) VALUES (?, ?, ?)",
                $info['name'],
                $info['date_begin'],
                $info['date_end']
            );
            ...
            $this->db->prepared_query(
                "INSERT INTO contest_has_bonus_pool (contest_id, bonus_pool_id) VALUES (?, ?)",
                $contestId,
                $poolId
            );
        }
    ...
    public function schedulePayout(\Twig\Environment $twig): int {
        ...
        $contests = array_map(function ($id) {
            return new \Gazelle\Contest($id);
        }, $this->db->collect(0));
        $totalParticipants = 0;
