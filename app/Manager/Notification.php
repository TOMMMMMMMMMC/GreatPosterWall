...
class Notification extends \Gazelle\Base {
    ...
    public function loadCollages() {
        ...
        if ($new > 0) {
            $this->create(
                self::COLLAGES,
                'You have ' . article($new) . ' new collage update' . plural($new),
                'userhistory.php?action=subscribed_collages',
                self::INFO
            );
        }
    }
    ...
    public function loadInbox() {
        ...
        if ($new > 0) {
            $this->create(
                self::INBOX,
                'You have ' . article($new) . ' new message' . plural($new),
                Inbox::getLinkQuick('inbox', $this->userInfo['ListUnreadPMsFirst'] ?? false),
                self::INFO
            );
        }
    }
