...
class TorrentLabel {
    ...
    protected function element($class, $text): string {
        return sprintf(
            '<strong class="torrent_label tooltip %s" title="%s" style="white-space: nowrap;">%s</strong>',
            $class,
            $text,
            $text
        );
    ...
    public function label(): string {
        ...
        if (isset($this->info['FreeTorrent'])) {
            ...
            } elseif ($this->info['FreeTorrent'] == '11') {
                $label[] = "<i class = 'fas fa-tag'></i>" . $this->element('tl_free tl_neutral', '25');
            } elseif ($this->info['FreeTorrent'] == '12') {
                $label[] = "<i class = 'fas fa-tag'></i>" . $this->element('tl_free tl_neutral', '50');
            } elseif ($this->info['FreeTorrent'] == '13') {
                $label[] = "<i class = 'fas fa-tag'></i>" . $this->element('tl_free tl_neutral', '75');
            }
        }
