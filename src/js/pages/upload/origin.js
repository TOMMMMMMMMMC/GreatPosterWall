...
function removeAllArtistFields() {
  $('#artistfields .artist').slice(1).remove()
  ArtistCount = 1
}
...
function artistsShowMore({ hide } = {}) {
  if (hide) {
    $('.artistfields .artist').slice(5).hide()
    $('.artistfields .show-more').show()
  } else {
    $('.artistfields .artist').slice(5).show()
    $('.artistfields .show-more').hide()
  }
}
...
function noImdbId() {
  ...
  form.toggleClass('no-imdb-id')
  $('.no-imdb-note').toggleClass('hidden')
}
