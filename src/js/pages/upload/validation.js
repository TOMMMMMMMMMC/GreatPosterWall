...
import { hasUnsupportedChars } from 'modules/Videoinfo/Videoinfo'
...
export function registerValidation() {
  ...
  const validator = new Pristine(form, {
    classTo: 'error-container',
    errorClass: 'form-invalid',
    successClass: 'form-valid',
    errorTextParent: 'error-container',
    errorTextTag: 'div',
  ...
  form.onsubmit = function (e) {
    ...
    document
      .querySelector('.form-invalid .error-message')
  ...
  addValidator({
    selector: `[name="mediainfo[]"]`,
    validate: validateMediainfoInvalidChars,
    messageKey: 'upload.mediainfo_invalid_chars',
  })
  addValidator({
    selector: `[name="mediainfo[]"]`,
    validate: validateMediainfoCompleteNameRequired,
...
export function validateMediainfoInvalidChars(value) {
  if (!value) {
    return true
  }
  if (hasUnsupportedChars(value)) {
    console.error('Mediainfo有不支持的字符.')
    return false
  }
  return true
}

export function validateMediainfoCompleteNameRequired(value) {
  if (!value) {
    return true
  }
  if (hasUnsupportedChars(value)) {
  ...
  const found = value.match(/(Complete name\s*:|Disc (Title|Label)\s*:)/i)
...
export function handleSourceAndProcessing() {
  const validator = document.querySelector('.form-validation').validator
