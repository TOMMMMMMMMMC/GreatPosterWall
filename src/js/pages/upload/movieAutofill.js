function movieAutofill() {
  function setLoading(loading) {
    if (loading) {
      $('.btn.autofill').addClass('is-loading').prop('disabled', true)
    } else {
      $('.btn.autofill').removeClass('is-loading').prop('disabled', false)
    }
  }
  function setError(key, values = []) {
    ...
    $('.imdb.error-message').html(key ? message : '')
  }
  ...
  $.ajax({
    ...
    success: (data) => {
      ...
      $('.form-validation')[0].validator.validate()
      $('.create_form').addClass('autofilled')
      $(
        '.artistfields input:not([name="artists_chinese[]"]), .artistfields select'
      ).prop('disabled', true)
      if (artists.length >= 5) {
